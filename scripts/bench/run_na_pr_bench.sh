#!/usr/bin/env bash
# Benchmark lanes for the bundled string/re/reprlib modules (PR #9440).
#
...
#
# Usage:  scripts/bench/run_na_pr_bench.sh   (JAC env var overrides the binary)
set -euo pipefail
cd "$(dirname "$0")/../.."
...
"$JAC" run -b python scripts/bench/bench_na_pr.jac
echo
echo "=== lane 4: jac native backend -> bundled na_stdlib ==="
"$JAC" run -b native scripts/bench/bench_na_pr.jac
