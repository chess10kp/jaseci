#!/usr/bin/env python3
"""Pinned host compilation for JacPython dataclass method generation.

Invoked as a subprocess so JacPython does not rely on the fused jac
interpreter's embedded CPython (which can lag the pinned 3.14.6 bytecode format).
"""
...
def resolve_cpython() -> Path:
    ...
    raise RuntimeError(
        f"pinned CPython {CPYTHON_PIN} not found for bootstrap compilation "
...
def main() -> None:
    if len(sys.argv) != 4:
        raise SystemExit("usage: host_compile_bridge.py <filename> <mode> <optimize>")
